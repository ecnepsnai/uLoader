...
function buildLinkRow(parentElem, linkId) {
    ...
    var fileDisplayNameCell = setCell('\u00a0\u00a0\u00a0' + $.trim(gLinksStorage.id2Name[linkId]),
        'linksTableCellClass');
...
function buildLinksTable(parentElem) {
    try {
        ...
        if (tblBodyElem.childElementCount < 2000) {
            $('#' + LINKS_TABLE_ID).colResizable(COL_RESIZER_ENABLE_PARAM);
        }
...
function fillFileSizeClmnRecurs(id) {
    ...
    if (gLinksStorage.getSize(id) !== undefined) {
        if (fillSizeRecursionStopFlag === false) {
            buildSizeCell(id, gLinksStorage.getSize(id));
        }
        ...
        $.ajax({
            ...
            success: function(message, text, response) {
                ...
                if (fileSize === null || fileSize === undefined) {
                    fileSize = BAD_SIZE_STRING;
                }
                gLinksStorage.addSize(id, fileSize);

                if (fillSizeRecursionStopFlag === false) {
                    buildSizeCell(id, fileSize);
                }
                setTimeout(function() { fillFileSizeClmnRecurs(++id); }, 1);
            },
            error: function() {
...
function addFileSizeClmn() {
    // disable col resizer effect
    $('#' + LINKS_TABLE_ID).colResizable(COL_RESIZER_DISABLE_PARAM);
    // remove table header
    $('#' + LINKS_TBL_HDR_ROW_ID).remove();
    // build table header
    buildLinksTableHeader($('#' + LINKS_TBL_BODY_ID)[0]);
    buildTableHeaderColumn($('#' + LINKS_TBL_HDR_ROW_ID)[0],
        // add the file size column
        'File size', 'linkTableHeaderStyle', LINKS_FILE_SIZE_CLMN_WIDTH);

    setTimeout(function() { fillFileSizeClmnRecurs(0); }, 1);

    // enable col resizer effect
    $('#' + LINKS_TABLE_ID).colResizable(COL_RESIZER_ENABLE_PARAM);
}
...
function removeFileSizeClmn() {
    ...
    // remove the last column which is the file size column
    $('#' + LINKS_TBL_HDR_ROW_ID + ' th:last-child').remove();

    // remove all the size column cells
    for (var i = 0; i < gLinksStorage.length(); ++i) {

        // in case user closed size column while getting sizes, to avoid overrun of the delition
        if ($('#' + LINK_ROW_ELEM_ID_PREFIX + i).children().length == 3) {
            continue;
        }
